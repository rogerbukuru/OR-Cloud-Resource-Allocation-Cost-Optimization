...
```{r}
# Enhanced Objective Function for GA
# Enhanced Objective Function for GA
...
## a. Single-Point Crossover for GA
...
# Order-Based Crossover for GA (Adapted) - Less applicable for Integer Assignments
...
# Partially Mapped Crossover (PMX) for GA (Adapted) Less applicable for integer assignments
...
## a. Mutation by Random Reassignment
...
```{r}
# 7. Genetic Algorithm Function
# 7. Genetic Algorithm Function with Multiple Crossover and Mutation Operators
...
```{r}
# 8. Execution with Multiple Crossover and Mutation Operators
...
# Assuming vm_cloud_clean_analysis is already loaded with necessary columns:
# - energy_efficiency
# - execution_time
# - cpu_usage
# - memory_usage
...
mutation_rate <- 0.01
tournament_size <- 3
crossover_type <- "n_point"  # Options: "single_point", "n_point", "uniform", "order_based", "pmx"
mutation_type <- "random_reassignment"        # Options: "random_reassignment", "swap", "inversion", "scramble", "insert"
generation_model <- "steady_state"  # Options: "generational", "steady_state"

# Run Genetic Algorithm with Multiple Operators
...
ggplot(data.frame(Generation = 1:num_generations, Fitness = ga_result$fitness_history), 
       aes(x = Generation, y = Fitness)) +
  geom_line(color = "darkgreen") +
  labs(title = "GA: Best Fitness Over Generations",
       x = "Generation",
       y = "Best Fitness") +
  theme_minimal()
...
# Create a bar plot for Task Distribution in Genetic Algorithm
ggplot(task_counts_ga, aes(x = factor(Assigned_Server), y = Num_Tasks)) +
